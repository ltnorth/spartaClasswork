...
class SeleniumDemoReg
  ...
  def get_selected_country
    ...
    options.find_elements(:tag_name, 'option').each do |element|
      return element.text if element.selected?
    end
  end
  ...
  def get_selected_month
    options = @chrome_driver.find_element(:id, @dob_month_dropdown_list)
    options.find_elements(:tag_name, 'option').each do |element|
      return element.text if element.selected?
    end
  end
  ...
  def get_selected_day
    options = @chrome_driver.find_element(:id, @dob_day_dropdown_list)
    options.find_elements(:tag_name, 'option').each do |element|
      return element.text if element.selected?
    end
  end
  ...
  def get_selected_year
    options = @chrome_driver.find_element(:id, @dob_year_dropdown_list)
    options.find_elements(:tag_name, 'option').each do |element|
      return element.text if element.selected?
    end
  end
  ...
  def click_submit
    ...
    # find_element(:name, @submit_button).click
  end

  def check_registration_successful
    @chrome_driver.find_element(:xpath, '//*[@id="post-49"]/div/p').text
  end
end
